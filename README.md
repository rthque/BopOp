...
- **Method statements** — per task: method statement (EN by default, 🇫🇷 toggle), tools & consumables, PPE & required trainings. Editable in admin mode. **Reached from the task itself**: tap any task in the left-panel *Tasks* list (in admin mode, the document button on its row) and its instruction opens, expanded and scrolled to. An instruction nobody has read yet is flagged with a red dot on its own row, not only on the top-bar counter. The top-bar button still opens the whole list, on whatever you were last reading.
