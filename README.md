...
- **Tasks (up to 56), on three rings** — a foundation is a dial: **8** slices in the centre, then a **16**-cell ring, then a **32**-cell ring. A ring is only drawn once there is something in it, so a farm with eight tasks looks exactly as it always did. In admin mode every row carries ↑ ↓ to reorder it — the list order **is** the order of the slices, clockwise from twelve, so moving a row moves the wedge — and a picker to send the task to another ring. **A task moved between rings takes its ticks with it**; nobody loses a morning of work to a layout decision. A full ring refuses the move and says so rather than dropping the task. The arrangement travels: order and ring are one layout decision, merged most-recently-arranged-wins, like the cable drawing.
- **One line between the bands.** Each ring starts exactly where the one inside it ends, so the two strokes fall on the same line and read as one. They used to be 3px apart, which drew a double line with a sliver of white trapped between. Every slice keeps its outline whether or not the task is done: dropping it on the untouched ones was tried and looked worse — the dial stopped reading as a dial.
- **Task states** — not done / **partially done (hatched fill)** / done. Every check is timestamped and attributed to the logged-in technician, and so is every *change* of state: "not done" carries no stamp of its own, so without a separate date the merge had nothing to tell "he unticked it" from "I have not seen this tick yet", and the other phone posted the tick straight back. Unticking now sticks, on every device. The same holds for taking back one occurrence of a repeatable report: occurrences stay a union — two techs each recording one offline must both count — so a removed one is remembered by name for 30 days instead.
...
- **Login screen** — deliberately anonymous: no name, no logo, no trade, nothing a stranger who lands on the address could use to tell what this is or whose it is. An abstract eight-part dial, the crew's names, and a **Visitor (read-only)** way in. One password for everyone, technicians and visitors alike — Visitor goes through the same door, because the site is meant to say nothing at all to someone without the word. Firebase refuses an account password under six characters, so the typed word is translated into the team account's own one just before the request. That translation hides nothing and is not meant to: every browser downloads `app.js`, so both words are in it in plain sight. **The password is a doorbell, not a lock.** What guards the crew's data is the database rule that refuses writes without a sign-in, and the fact that nobody outside the team knows the address. The name of whoever validates a task is shown in grey next to it.
...
- **Method statements** — per task: **communication / report** (who to tell and by which route when the job turns up a punch — first, because it is what you need before starting), method statement (EN by default, 🇫🇷 toggle), tools & consumables, PPE & required trainings. Editable in admin mode. **Reached from the task itself**: tap any task in the left-panel *Tasks* list (in admin mode, the document button on its row) and its instruction opens, expanded and scrolled to. There is no separate button in the top bar — an isolated icon nobody finds is worse than no icon. An instruction this person has not read yet is flagged with a red dot on its own row, and the count of updated instructions rides on the button that opens the task list.
- **What this app can do** — a `?` in the top bar opens a guide written to be *read*, not tapped: who can do what at each of the three levels, then the map, the task list, a foundation card, the day-to-day tools and what happens to your data — six sections, twenty points, in English or French on the same switch the method statements use. It is there for the person who opens the app and wonders what else is in it.
...
- **Toolbox talk of the day** — right panel → *TBT & published recaps*. Write the day's talk; **the date is the archive**, so each day keeps its own entry and today's can never overwrite yesterday's. Nobody has to remember to file anything. Two people writing the same day land on the same entry and the later one wins. Kept for a year and a bit.
- **What went out to WhatsApp, kept** — every 12h recap that was copied is stored word for word, with who sent it and when, and can be copied again. The recap is built from a moving twelve-hour window, so it cannot be rebuilt afterwards: if it is not kept at the moment it goes out, it is gone.
...
- **Which foundations are left** — every task row carries a button that answers the question this list gets asked all day. It opens the list of foundations still to do, with a tab for the ones already done, and a part-done foundation is striped rather than lumped in with the untouched ones. Tap any of them to open that foundation. Before this it was answered by squinting at the map one wedge at a time.
- **A punch belongs to a foundation.** The free-standing box that added a punch entry from nowhere is gone; a punch is raised from the foundation card, so it always says which one it is about.
...
**The word typed on the login screen is not this account's password.** The door checks the crew word itself, then signs in to the team account with the account's own password. Typing the account's password does NOT open the door — the crew word is required first.

Both words sit in `app.js`, which every browser downloads. That is unavoidable in a site with no backend, and it is the reason the password is a doorbell and not a lock: it stops the passer-by, not somebody reading the page source. Real protection is one account per person checked on a server — the v2 job.

An old crew word is turned away: leaving it accepted would make changing the password decorative. A device that was already signed in before the change is asked for the new word on its next load.
...
- Changing the account password in Firebase revokes every device on their next token refresh — so it must be changed in the console and in `TEAM_SECRET` together, in that order.
