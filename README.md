...
- **Ink only where there is work.** The three bands touch, so one black line separates them instead of two strokes with a sliver of white trapped between. And a slice with nothing done carries no outline at all — plain paper. The black line only ever closes around a task that is partially or fully done, so the eye finds the colour instead of hunting for it inside a grid. An unlined slice is still painted, so it is still a live target for a gloved tap.
