...
- **The map cannot be panned away.** When the farm is smaller than the view it stays centred; when it is bigger you can pan but never past the outermost foundation. No foundation can be dragged off-screen at any zoom. Any string can be flagged **SRCC** (restricted): its cable becomes a bold red run, every foundation it feeds is ringed in red, and a restricted-access reminder is shown on the string panel and on each of those foundations.
